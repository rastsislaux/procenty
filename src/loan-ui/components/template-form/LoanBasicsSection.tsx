...
export function LoanBasicsSection({ value, onChange }: { value: Template; onChange: (tpl: Template) => void }) {
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      <div>
        <FormLabel className="text-xs sm:text-sm">{t.templates.name}</FormLabel>
        <FormInput value={value.name} onChange={(e) => update('name', e.target.value)} required />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
        <div>
          <FormLabel className="text-xs sm:text-sm">{t.templates.currency}</FormLabel>
          <Select options={currencies.map((c) => ({ value: c, label: c }))} value={value.currency ?? 'USD'} onChange={(v) => update('currency', v as any)} />
        </div>
        <div>
          <FormLabel className="text-xs sm:text-sm">{t.templates.amortization}</FormLabel>
        ...
        <div>
          <FormLabel className="text-xs sm:text-sm">{t.templates.dayCount}</FormLabel>
          <Select options={dayCounts.map((d) => ({ value: d, label: d }))} value={value.dayCount ?? '30E_360'} onChange={(v) => update('dayCount', v as any)} />
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
        <div>
          <FormLabel className="text-xs sm:text-sm">{t.templates.rate}</FormLabel>
        ...
        <div>
          <FormLabel className="text-xs sm:text-sm">{t.templates.termMonths}</FormLabel>
          <FormInput type="number" value={value.termMonths ?? ''} onChange={(e) => update('termMonths', e.target.value === '' ? undefined : Number(e.target.value))} />
        </div>
        <div>
          <FormLabel className="text-xs sm:text-sm">{t.templates.prepaymentPolicy}</FormLabel>
          <Select
            options={[
              { value: 'ReduceTerm' as const, label: t.templates.reduceTerm },
              { value: 'ReduceInstallment' as const, label: t.templates.reduceInstallment },
            ]}
