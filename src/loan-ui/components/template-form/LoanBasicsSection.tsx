  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <FormLabel className="text-sm">{t.templates.dayCount}</FormLabel>
          <Select options={dayCounts.map((d) => ({ value: d, label: d }))} value={value.dayCount ?? '30E_360'} onChange={(v) => update('dayCount', v as any)} />
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <FormLabel className="text-sm">{t.templates.rate}</FormLabel>
        ...
        <div>
          <FormLabel className="text-sm">{t.templates.termMonths}</FormLabel>
          <FormInput type="number" value={value.termMonths ?? ''} onChange={(e) => update('termMonths', e.target.value === '' ? undefined : Number(e.target.value))} />
        </div>
        <div>
          <FormLabel className="text-sm">{t.templates.prepaymentPolicy}</FormLabel>
          <Select
            options={[
